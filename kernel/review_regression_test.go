...
func TestGovernRejectsMismatchedDecision(t *testing.T) {
	...
	if _, err := r.Normalize(Proposal{Subject: "resource", TargetState: "B"}); err != nil { t.Fatal(err) }
	if _, err := r.Reconcile(); err != nil { t.Fatal(err) }
	if _, err := r.Govern(); !errors.Is(err, ErrGovernanceDenied) { t.Fatalf("expected governance denial, got %v", err) }
}
...
func TestObserveRejectedDuringRecovery(t *testing.T) {
	...
	if err := r.Observe(fresh); !errors.Is(err, ErrInvalidLifecycle) { t.Fatalf("expected recovery observation to be rejected, got %v", err) }
	if r.Phase() != PhaseRecovery { t.Fatalf("expected recovery phase, got %s", r.Phase()) }
}
...
func TestRecoverRejectsUnrelatedSubject(t *testing.T) {
	...
	if err := r.Recover(unrelated); !errors.Is(err, ErrInvalidObservation) { t.Fatalf("expected unrelated recovery evidence to be rejected, got %v", err) }
}
...
func TestVerifyCancellationReleasesReservation(t *testing.T) {
	...
	cancel()
	if err := r.Verify(ctx, fakeVerifier{}); !errors.Is(err, context.Canceled) { t.Fatalf("expected cancellation, got %v", err) }
	if r.Phase() != PhaseStarted { t.Fatalf("expected started phase after cancellation, got %s", r.Phase()) }
	close(exec.release)
	if err := <-startDone; err != nil { t.Fatal(err) }
}
