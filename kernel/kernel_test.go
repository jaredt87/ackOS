...
func TestAuthorityCannotBeReplayed(t *testing.T) {
	...
	if a.Consumed {
		t.Fatal("returned authority must be an immutable pre-consumption snapshot")
	}
	if r.Phase() != PhaseRecovery {
		t.Fatalf("expected recovery, got %s", r.Phase())
	}
...
func TestObserveRejectedWhileExecutionInFlight(t *testing.T) {
	...
	if r.Phase() != PhaseStarted {
		t.Fatalf("expected started phase, got %s", r.Phase())
	}
	close(exec.release)
...
func TestVerifyWaitsForExecutionCompletion(t *testing.T) {
	...
	post := observation(t, "resource", "B", 2, postTime)
	verified := make(chan error, 1)
	go func() {
		verified <- r.Verify(context.Background(), fakeVerifier{observation: post, called: called, release: release})
	}()
	select {
	case <-called:
	...
	clockNow = postTime.Add(time.Second)
	close(exec.release)
	if err := <-startDone; err != nil {
		t.Fatal(err)
	}
...
func TestConcurrentVerifyAllowsOnlyOneVerifier(t *testing.T) {
	...
	clockNow = postTime.Add(time.Second)
	post := observation(t, "resource", "B", 2, postTime)
	first := &blockingVerifier{entered: make(chan struct{}), release: make(chan struct{}), observation: post}
...
func TestFailedExecutionCannotBeVerified(t *testing.T) {
	...
	if err := r.Verify(context.Background(), fakeVerifier{}); !errors.Is(err, ErrInvalidLifecycle) {
		t.Fatalf("expected verification to be rejected, got %v", err)
	}
}
...
func TestRecoverRequiresFreshEvidence(t *testing.T) {
	...
	oldVersion := observation(t, "resource", "A", 2, now.Add(-time.Second))
	if err := r.Recover(oldVersion); !errors.Is(err, ErrStaleEvidence) {
		t.Fatalf("expected pre-attempt evidence to be rejected, got %v", err)
	}
	freshTime := now.Add(time.Second)
	clockNow = freshTime
	fresh := observation(t, "resource", "A", 2, freshTime)
	if err := r.Recover(fresh); err != nil {
		t.Fatalf("expected fresh recovery evidence to be accepted, got %v", err)
	}
