...
func TestExecutorRejectsCleanFilterBeforeStatus(t *testing.T) {
	...
	gitTest(t, target.Repository, "config", "filter.evil.required", "false")
	gitTest(t, target.Repository, "config", "filter.evil.process", "")
	gitTest(t, target.Repository, "config", "filter.evil.clean", "sh -c 'echo side-effect > ../filter-side-effect; cat'")
	gitTest(t, target.Repository, "config", "filter.evil.smudge", "cat")
	gitTest(t, target.Repository, "attr.docs/example.md.filter", "evil")
	result := executor.Execute(context.Background(), kernel.Transition{Subject: target.Subject, Before: "initial", After: "updated"}, kernel.Authority{ExecutionID: "filter-before-status"})
	if result.Success { t.Fatal("executor accepted configured clean filter") }
	if !strings.Contains(result.Message, "clean filter") && !strings.Contains(result.Message, "filter") { t.Fatalf("unexpected error: %s", result.Message) }
	if _, err := os.Stat(filepath.Join(filepath.Dir(target.Repository), "filter-side-effect")); err == nil { t.Fatal("clean filter ran before rejection") }
}
...
func TestRunGitDisablesFSMonitor(t *testing.T) {
	...
	if _, err := runGit(context.Background(), target.Repository, "status", "--porcelain"); err != nil { t.Fatal(err) }
}
...
func TestCommitVerificationUsesRealObjects(t *testing.T) {
	...
	if result := executor.Execute(context.Background(), transition, authority); !result.Success { t.Fatal(result.Message) }
	if _, err := verifier.Verify(context.Background(), transition, authority); err != nil { t.Fatal(err) }
}
