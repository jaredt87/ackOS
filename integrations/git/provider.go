...
func rejectConfiguredFilters(ctx context.Context, target Target) error {
	...
	cleanDrivers, cleanErr := runGit(ctx, target.Repository, "config", "--name-only", "--get-regexp", `^filter\..*\.clean// Package git provides a domain-specific Git resource provider for ackOS.
package git
